...
class RNDAgent(DQNAgent):
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        target_pred = self.rnd_target_net(obs)
        pred = self.rnd_net(obs)
        loss = torch.mean((pred - target_pred) ** 2)
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            target_pred = self.rnd_target_net(observations)
            pred = self.rnd_net(observations)
            rnd_error = torch.norm(pred - target_pred, dim=-1)
